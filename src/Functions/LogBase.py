...
class CustomFormatter(logging.Formatter):
    ...
    bold_red = "\x1b[31;1m"
    reset = "\x1b[0m"
    format = "[%(asctime)s %(name)s:%(levelname)s] %(message)s - (%(filename)s:%(lineno)d)(%(process)d:%(thread)d:%(threadName)s)"  # noqa: E501
...
def get_logger(name: str, level=logging.DEBUG, write_file=False):
